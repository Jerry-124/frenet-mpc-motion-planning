...
def _actual_minimum_clearance(road, states, obstacles, dt):
    frenet = np.asarray(
        [road.cartesian_to_frenet(state[0], state[1]) for state in states]
    )
...
def run_fallback_demo(
    ...
    config_path: Path = Path("configs/fallback_demo.json"),
) -> dict:
    project = load_project_config(config_path)
    sim_cfg, mpc_cfg, vehicle_cfg, settings = (
        project.simulation,
        project.mpc,
        project.vehicle,
        project.raw["planner"],
    ...
    if decision.mode != "emergency_fallback":
        raise RuntimeError(
            "The blocked-road validation scenario did not trigger fallback"
        )

    tracking = track_reference(
        "emergency_brake",
        road,
        decision.trajectory.states,
        sim_cfg,
        initial_lateral_offset=project.road.initial_lateral_offset_m,
        vehicle_cfg=project.vehicle,
        mpc_cfg=project.mpc,
    )
    actual_clearance, actual_frenet = _actual_minimum_clearance(
        road, tracking.states, obstacles, sim_cfg.dt
    )
    ...
    summary = {
        ...
        "rejected_candidate_count": sum(
            not candidate.feasible for candidate in candidates
        ),
...
def _save_plot(path, road, candidates, emergency, obstacles, states, dt):
    ...
    fig, axes = plt.subplots(
        2, 1, figsize=(11, 8), gridspec_kw={"height_ratios": [2.0, 1.0]}
    )
    ax = axes[0]
    ax.plot(road.x, road.y, "--", color="0.7", label="road centerline")
    for candidate in candidates:
        ax.plot(
            candidate.trajectory.x,
            candidate.trajectory.y,
            color="tab:red",
            alpha=0.18,
            linewidth=1.0,
        )
    ...
    ax.plot(
        ...
        color="tab:blue",
        linewidth=3.0,
        label="emergency-stop reference",
    )
    ax.plot(
        states[:, 0],
        states[:, 1],
        color="tab:orange",
        linewidth=2.0,
        label="safety-controller tracking",
    )
    for index, obstacle in enumerate(obstacles):
        ...
        ax.scatter(
            ...
            color="black",
            zorder=5,
            label="blocked lanes" if index == 0 else None,
        )
    ax.set(
        xlabel="x [m]",
        ylabel="y [m]",
        title="No feasible candidate: emergency fallback",
    )
    ...
    axes[1].plot(
        reference_time, emergency.speed, linewidth=2.5, label="emergency reference"
    )
    ...
    axes[1].set(
        xlabel="time [s]",
        ylabel="speed [m/s]",
        title="Maximum-deceleration stop profile",
    )
    axes[1].grid(True)
    axes[1].legend()
    fig.tight_layout()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run configured emergency-fallback validation"
    )
    parser.add_argument(
        "--config", type=Path, default=Path("configs/fallback_demo.json")
    )
