...
def save_plots(directory: Path, road: ReferencePath, reference: np.ndarray, states: np.ndarray, controls: np.ndarray, dt: float):
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(directory / "reference_vs_actual.png", dpi=160)
    plt.close(fig)
    ...
    ax.plot(time, error)
    ax.set(xlabel="time [s]", ylabel="position error [m]", title="Tracking error")
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(directory / "tracking_error.png", dpi=160)
    plt.close(fig)

    control_time = time[:-1]
    fig, axes = plt.subplots(2, 1, figsize=(9, 6), sharex=True)
    axes[0].plot(control_time, controls[:, 0])
    axes[0].set_ylabel("acceleration [m/s²]")
    axes[0].grid(True)
    axes[1].plot(control_time, np.rad2deg(controls[:, 1]))
    axes[1].set_ylabel("steering [deg]")
    axes[1].set_xlabel("time [s]")
    axes[1].grid(True)
    fig.tight_layout()
    fig.savefig(directory / "control_inputs.png", dpi=160)
    plt.close(fig)
