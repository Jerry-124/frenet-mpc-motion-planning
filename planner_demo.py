...
def run_planner_demo(
    output_dir: Path = Path("results"),
    config_path: Path = Path("configs/planner_demo.json"),
):
    ...
    save_candidate_table(
        output_dir / "metrics" / "frenet_candidates.csv", candidates, selected
    )
    save_planning_plot(
        output_dir / "figures" / "frenet_candidate_selection.png",
        road,
        candidates,
        selected,
        obstacles,
...
def save_planning_plot(
    path: Path, road, candidates, selected, obstacles, tracked_states, road_bounds
):
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for index, obstacle in enumerate(obstacles):
        ...
        ax.scatter(
            ...
            marker="X",
            color="black",
            label="slow obstacle (initial)" if index == 0 else None,
            zorder=5,
        )
        obstacle_s = obstacle.s + obstacle.speed * selected.trajectory.time
        ox, oy = road.frenet_to_cartesian(
            obstacle_s, np.full_like(obstacle_s, obstacle.d)
        )
        ax.plot(
            ox,
            oy,
            "k:",
            linewidth=1.5,
            label="obstacle prediction" if index == 0 else None,
        )
    ax.set(
        xlabel="x [m]",
        ylabel="y [m]",
        title="Frenet candidate generation, rejection, and selection",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run configured Frenet candidate planning demo"
    )
    parser.add_argument(
        "--config", type=Path, default=Path("configs/planner_demo.json")
    )
