...
def run_benchmark(
    ...
    config_path: Path = Path("configs/benchmark.json"),
) -> list[dict]:
...
def save_comparison_plots(
    ...
    for offset, controller in zip((-width / 2, width / 2), controllers):
        values = [
            row["lateral_rmse_m"] for row in rows if row["controller"] == controller
        ]
    ...
    ax.set(
        xlabel="x [m]",
        ylabel="y [m]",
        title=f"Trajectory comparison: {comparison_scenario}",
    )
    ax.axis("equal")
    ax.grid(True)
    ax.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run NMPC vs Stanley/PID scenario benchmark"
    )
