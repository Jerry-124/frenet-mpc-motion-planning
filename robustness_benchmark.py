...
def run_trial(scenario: RobustnessScenario, seed: int, project=None):
    ...
    for index in range(simulation_steps):
        ...
        measured[3] = max(
            0.0,
            measured[3] + rng.normal(0.0, scenario.speed_noise_std_mps),
        )
        ...
        if scenario.delay_compensation and scenario.actuation_delay_steps:
            ...
            for pending_control in actuator.pending_controls():
                controller_state = prediction_model.step_array(
                    controller_state,
                    pending_control,
                )
...
def run_robustness_benchmark(
    ...
    _save_summary_plot(
        output_dir / "figures" / "robustness_benchmark.png",
        summaries,
        float(acceptance["lateral_rmse_limit_m"]),
    )
    _save_trajectory_plot(
        output_dir / "figures" / "robustness_trajectories.png",
        representatives,
    )
...
def _group_rows(rows: list[dict], scenario_name: str) -> list[dict]:
    return [row for row in rows if row["scenario"] == scenario_name]


def _metric_values(group: list[dict], name: str) -> np.ndarray:
...
def _scenario_summary(
    ...
    group: list[dict],
    lateral_rmse_limit_m: float,
    max_position_error_limit_m: float,
) -> dict:
    lateral = _metric_values(group, "lateral_rmse_m")
    maximum = _metric_values(group, "max_position_error_m")
    violations = sum(int(row["constraint_violations"]) for row in group)
    failures = sum(int(row["solver_failures"]) for row in group)
    passed = bool(
...
def summarize(
    ...
    return [
        _scenario_summary(
            scenario,
            _group_rows(rows, scenario.name),
            lateral_rmse_limit_m,
            max_position_error_limit_m,
        )
        for scenario in scenarios
    ]
...
def _save_summary_plot(
    ...
    ax.set(
        ...
        title="NMPC robustness benchmark (mean ± std, 5 seeds)",
    )
...
def _save_trajectory_plot(path: Path, representatives: dict):
    ...
    ax.set(
        xlabel="x [m]",
        ylabel="y [m]",
        title="Representative trajectories under disturbances",
    )
    ax.axis("equal")
    ax.grid(True)
    ax.legend(ncol=2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run configured robustness Monte Carlo benchmark"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/robustness_benchmark.json"),
    )
