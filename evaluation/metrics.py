...
def calculate_metrics(
    ...
    yaw_error = np.arctan2(
        np.sin(states[:, 2] - reference[:, 2]), np.cos(states[:, 2] - reference[:, 2])
    )
    ...
    if dt is not None and len(controls):
        ...
        if max_steer_rate is not None:
            steering_rate_violations = int(
                np.sum(steering_rates > max_steer_rate + 1e-9)
            )
    violations = (
        acceleration_violations
        + steering_violations
        + speed_violations
        + steering_rate_violations
    )
    return {
        ...
        "speed_rmse_mps": float(
            np.sqrt(np.mean((states[:, 3] - reference[:, 3]) ** 2))
        ),
