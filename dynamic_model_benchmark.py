...
def _load_scenarios(items, project) -> tuple[DynamicScenario, ...]:
    return tuple(DynamicScenario(
        name=item["name"], speed_mps=item["speed_mps"],
        ...
        lane_change_duration_s=item.get("lane_change_duration_s", project.simulation.lane_change_duration),
...
def run_dynamic_trial(scenario: DynamicScenario, project=None):
    ...
    sim_cfg = replace(project.simulation, target_speed=scenario.speed_mps)
    if scenario.plant == "kinematic":
        result = run_closed_loop(
            "nmpc", sim_cfg, road_amplitude=scenario.road_amplitude_m,
            initial_lateral_offset=settings["initial_lateral_offset_m"],
            vehicle_cfg=project.vehicle, mpc_cfg=project.mpc,
            road_wavelength=project.road.wavelength_m, road_length=project.road.length_m,
        )
        return {
            **result.metrics,
        ...
        }, result.states, result.reference

    limits, mpc_cfg = project.vehicle, project.mpc
    road = ReferencePath.sinusoidal(
        length=project.road.length_m, amplitude=scenario.road_amplitude_m,
    ...
    if scenario.friction_aware_planner:
        planning_result = select_friction_aware_trajectory(
            road=road,
            duration=sim_cfg.duration + mpc_cfg.horizon * sim_cfg.dt,
            dt=sim_cfg.dt,
            ...
            target_speeds=tuple(settings["friction_planner"]["target_speeds_mps"]),
            lane_change_durations=tuple(settings["friction_planner"]["lane_change_durations_s"]),
            lane_change_start_times=tuple(settings["friction_planner"]["lane_change_start_times_s"]),
            speed_transition_durations=tuple(settings["friction_planner"]["speed_transition_durations_s"]),
            safety_factor=settings["friction_planner"]["safety_factor"],
            min_longitudinal_acceleration=limits.min_accel,
            max_longitudinal_acceleration=limits.max_accel,
            cost_weights=friction_plan_cost_weights_from_mapping(
                settings["friction_planner"].get("cost_weights")
            ),
        )
        trajectory = planning_result.trajectory
    else:
        trajectory = generate_lane_change(
            road, sim_cfg.duration + mpc_cfg.horizon * sim_cfg.dt, sim_cfg.dt,
            sim_cfg.target_speed, sim_cfg.lane_width, scenario.lane_change_duration_s,
        )
    ...
    controller = NonlinearMPC(
        ...
        limits,
        mpc_cfg,
        max_steer_rate=scenario.max_steer_rate_rad_s if scenario.rate_aware_controller else None,
    )
    if settings["vehicle_model"]["tire_model"] != "smooth_tanh_friction_circle":
        raise ValueError("Unsupported dynamic tire model")
    plant_config = DynamicVehicleConfig(
        mass=settings["vehicle_model"]["mass_kg"],
        yaw_inertia=settings["vehicle_model"]["yaw_inertia_kgm2"],
        lf=settings["vehicle_model"]["lf_m"],
        lr=settings["vehicle_model"]["lr_m"],
        cornering_stiffness_front=settings["vehicle_model"]["cornering_stiffness_front_n_rad"],
        cornering_stiffness_rear=settings["vehicle_model"]["cornering_stiffness_rear_n_rad"],
        friction_coefficient=scenario.friction_coefficient,
        gravity=settings["vehicle_model"]["gravity_mps2"],
        integration_substeps=settings["vehicle_model"]["integration_substeps"],
        max_steer_rate_rad_s=scenario.max_steer_rate_rad_s,
    )
    plant = DynamicBicycle(plant_config, limits, sim_cfg.dt)
    initial = trajectory.states[0].copy()
    initial[1] += (
        settings["friction_aware_initial_lateral_offset_m"]
        if scenario.friction_aware_planner else settings["initial_lateral_offset_m"]
    )
    state = DynamicVehicleState(initial[0], initial[1], initial[2], initial[3])
    ...
    for index in range(simulation_steps):
        ...
        states.append(state.as_array()); controls.append(control)
    ...
    metrics = calculate_metrics(
        controller_states, reference, controls, solver_failures, limits, sim_cfg.dt,
        scenario.max_steer_rate_rad_s,
    )
    sideslip = np.arctan2(states[:, 4], np.maximum(states[:, 3], 0.1))
    lateral_accel = np.gradient(states[:, 4], sim_cfg.dt) + states[:, 3] * states[:, 5]
    metrics.update({
        ...
        "selected_target_speed_mps": scenario.speed_mps if planning_result is None else planning_result.target_speed,
        "selected_lane_change_duration_s": scenario.lane_change_duration_s if planning_result is None else planning_result.lane_change_duration,
        "selected_lane_change_start_time_s": 0.0 if planning_result is None else planning_result.lane_change_start_time,
        "selected_speed_transition_duration_s": 0.0 if planning_result is None else planning_result.speed_transition_duration,
        "friction_accel_budget_mps2": float("nan") if planning_result is None else planning_result.lateral_acceleration_budget,
        "reference_peak_lateral_accel_mps2": float("nan") if planning_result is None else planning_result.peak_lateral_acceleration,
        "reference_peak_longitudinal_accel_mps2": float("nan") if planning_result is None else planning_result.peak_longitudinal_acceleration,
        "reference_peak_combined_accel_mps2": float("nan") if planning_result is None else planning_result.peak_combined_acceleration,
    })
...
def run_dynamic_benchmark(
    ...
    with (metrics_dir / "dynamic_model_benchmark.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
        writer.writeheader(); writer.writerows(rows)
    _save_metric_plot(output_dir / "figures" / "dynamic_model_benchmark.png", rows, lateral_limit)
    _save_trajectory_plot(
        output_dir / "figures" / "dynamic_model_trajectories.png",
        trajectories, rows, project.simulation.dt,
    )
...
def _save_metric_plot(path: Path, rows: list[dict], lateral_limit: float):
    ...
    ax.axhline(lateral_limit, color="black", linestyle="--", label="RMSE acceptance limit")
    ax.set_yscale("log")
    ax.set(ylabel="lateral RMSE [m] (log scale)", title="Kinematic MPC controlling kinematic vs. dynamic plants")
    ax.set_xticks(np.arange(len(rows)), labels); ax.tick_params(axis="x", labelsize=8)
...
def _save_trajectory_plot(path: Path, trajectories: dict, rows: list[dict], dt: float):
    ...
    for name, (states, reference) in trajectories.items():
        ...
        axes[0 if status[name] else 1].plot(time, lateral_error, linewidth=1.5, label=name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/dynamic_model_benchmark.json"))
